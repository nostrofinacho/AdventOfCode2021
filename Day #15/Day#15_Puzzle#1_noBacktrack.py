...
with open("Day#15_Puzzle#1_Input.txt") as input_file:
    ...
    def search_around(loc):
        ...
        for i, j in [(loc[0]-1, loc[1]), (loc[0]+1, loc[1]), (loc[0], loc[1]-1), (loc[0], loc[1]+1)]:
            if 0 <= i < len(grid[0]) and 0 <= j < len(grid):
                lit.append((i, j))
        return lit
    return [(loc, node[1] + grid[loc[0]][loc[1]]) for loc in search_around(node[0])]


# Performs an A* state search
# Node format -> (grid_location, total_path_cost)
def pathfinder(start_loc):
    ...
    while len(open) != 0:
        ...
        for child in get_succesors(node):
            ...
            better = [m for m in open + closed if m[0] == child[0] and m[1] < child[1]]
            worse = [m for m in open + closed if m[0] == child[0] and m[1] >= child[1]]
            ...
            if len(better) == 0:
                open.append(child)
                open = sorted(open, key=lambda x: x[1] + get_h(x), reverse=False)
    return closed


end_goal = pathfinder((0, 0))
print("Total path risk of the lowest risk path:", end_goal[1])
